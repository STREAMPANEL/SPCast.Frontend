        ...
        <div class="card-title">
            <h3 class="card-label text-white">
                <i class="fa fa-info text-danger mr-5" data-toggle="sp_popover" data-trigger="click" title="{lang key='sp_spcast_note'}"
                    data-content="{lang key='sp_spcast_loadbalancer_settings_datacontent'}">
                </i> {lang key='sp_spcast_loadbalancer_settings'}<br />
                <small>{lang key='sp_spcast_loadbalancer_settings_desc'}</small>
            </h3>
        </div>
    </div>
    <form method="post" action="{$smarty.server.PHP_SELF}?m=SPStreamserverManagement&spserviceid={$spServiceID}&spsmpageid=saved" role="form">
        ...
        <div class="card-footer">
            <div class="row col-xl-12">
                <input type="submit" class="btn btn-success btn-block" name="submit_settings_loadbalancer" value="{lang key='sp_spcast_settings_save'}" />
            </div>
        </div>
    </form>
